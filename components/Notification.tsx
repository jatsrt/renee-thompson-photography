...
export const NotificationContext = React.createContext({
  setShow: (val: boolean) => {},
  setMsg: ({ title, body }: { title: string; body: string }) => {},
});

const Notification: React.FC<React.PropsWithChildren> = ({ children }) => {
  const [show, setShow] = React.useState(false);
  const [msg, setMsg] = React.useState({ title: "", body: "" });

  return (
    <NotificationContext.Provider value={{ setShow, setMsg }}>
      {children}
      <div
        aria-live="assertive"
